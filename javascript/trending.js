...
const loadLatestQuestion = async () => {
    ...
    try {
        // গত ৭২ ঘন্টার টাইমস্ট্যাম্প তৈরি করা হচ্ছে
        ...
        if (questionData) {
            questionData.sort((a, b) => {
                const scoreA = (a.answer?.[0]?.count || 0) + (a.views || 0) + (a.votes || 0);
                const scoreB = (b.answer?.[0]?.count || 0) + (b.views || 0) + (b.votes || 0);
                return scoreB - scoreA;
            });
            questionData.splice(PAGE_SIZE);
        }
        ...
        if (questionData && questionData.length > 0) {
            ...
            questionList.innerHTML = '<p class="p-6 text-center text-gray-500 text-[13px]">গত ৭২ ঘন্টায় কোনো প্রশ্ন পাওয়া যায়নি।</p>';
        }
    } catch (err) {
