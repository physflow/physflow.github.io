...
async function buildAnswerThread(answer, depth) {
    ...
    actionRow.innerHTML = `
        ...
        <span class="w-px h-3 bg-gray-200 dark:bg-gray-800 mx-1.5 shrink-0"></span>
        <button class="ans-reply action-btn px-2">
            <i class="fas fa-reply" style="font-size:10px"></i> রিপ্লাই
        </button>`;

    // ── Reply compose ─────────────────────────
    const compose = document.createElement('div');
    compose.className = 'reply-compose hidden';
    compose.id        = `compose-${answer.id}`;
    compose.innerHTML = `
        <div class="rq-host mb-2 rounded border border-gray-200 dark:border-[#1e1e1e] overflow-hidden"></div>
        <div class="flex items-center gap-2 mt-1.5">
            <button class="rs-sub bg-[#0056b3] hover:bg-[#004494] text-white px-3 py-1 rounded text-xs
                           font-medium transition disabled:opacity-50 flex items-center gap-1.5">
                <span class="rs-txt">রিপ্লাই</span>
                <span class="rs-spin hidden"><i class="fas fa-spinner fa-spin" style="font-size:10px"></i></span>
            </button>
            <button class="rs-can text-xs text-gray-400 hover:text-gray-700 dark:hover:text-gray-300
                           px-2 py-1 rounded hover:bg-gray-100 dark:hover:bg-[#1c1c1c] transition">বাতিল</button>
            <span class="rs-err hidden text-red-500 text-xs">কমপক্ষে ৩০ অক্ষর লিখুন।</span>
        </div>`;
    ...
    rplBtn.addEventListener('click', () => {
        const open = !compose.classList.contains('hidden');
        compose.classList.toggle('hidden', open);
        if (!open) initReplyQuill(answer.id, compose);
    });
...
function initReplyQuill(answerId, composeEl) {
    ...
    const host = composeEl.querySelector('.rq-host');
    ...
    if (document.documentElement.classList.contains('dark')) {
        const c = host.querySelector('.ql-container');
        const t = host.querySelector('.ql-toolbar');
        if (c) { c.style.background='#0d0d0d'; c.style.color='#e5e5e5'; c.style.borderColor='#1e1e1e'; }
        if (t) { t.style.background='#151515'; t.style.borderColor='#1e1e1e'; }
    }

    replyQuillMap.set(answerId, q);

    const subBtn = composeEl.querySelector('.rs-sub');
    const canBtn = composeEl.querySelector('.rs-can');
    const errMsg = composeEl.querySelector('.rs-err');
    const spin   = composeEl.querySelector('.rs-spin');
    const txt    = composeEl.querySelector('.rs-txt');

    subBtn.addEventListener('click', async () => {
        if (!state.currentUserId) { showLoginToast(); return; }
        const html = q.root.innerHTML;
        const text = q.getText().trim();
        if (text.length < MIN_ANSWER_LENGTH) { errMsg.classList.remove('hidden'); return; }
        errMsg.classList.add('hidden');

        subBtn.disabled = true;
        spin.classList.remove('hidden');
        txt.textContent = 'পাঠানো হচ্ছে...';
        ...
        txt.textContent = 'রিপ্লাই';

        if (error) { console.error(error); showToast('রিপ্লাই পাঠাতে সমস্যা।'); return; }
    ...
    canBtn.addEventListener('click', () => composeEl.classList.add('hidden'));
}
