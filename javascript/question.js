...
const state = {
    ...
    currentCommentAnswerId: null,
    currentCommentAnswer: null,
};
...
async function openCommentModal(answer) {
    state.currentCommentAnswerId = answer.id;
    state.currentCommentAnswer = answer;
    ...
    const preview = document.getElementById('comment-answer-preview');
...
function closeCommentModal() {
    ...
    setTimeout(() => {
        ...
        state.currentCommentAnswerId = null;
        state.currentCommentAnswer = null;
    }, 250);
}
...
async function loadComments(answerId) {
    ...
    try {
        ...
        for (const comment of comments) {
            const commentEl = await buildCommentItem(comment);
        ...
        loadingEl.classList.add('hidden');
        showToast('মতামত লোড করতে সমস্যা হয়েছে।');
    }
}
...
async function buildCommentItem(comment) {
    ...
    const authorName = await fetchName(comment.author_id);
    
    // Header: author + time
    const header = document.createElement('div');
    header.className = 'flex items-center gap-2 mb-1.5';
    header.innerHTML = `
        <a href="user.html?id=${comment.author_id}"
           class="text-[12px] font-semibold text-[#0056b3] hover:underline">${escapeHTML(authorName)}</a>
        <span class="text-[11px] text-gray-400 dark:text-gray-600">${timeAgo(comment.created_at)}</span>
    `;
    
    // Body
    const body = document.createElement('div');
    body.className = 'text-[13px] text-gray-700 dark:text-gray-300 leading-relaxed';
...
async function submitComment() {
    ...
    const btnSpinner = document.getElementById('comment-btn-spinner');
    
    const text = input.value.trim();
    
    // Validate minimum length
    if (text.length < 1) {
        errorText.textContent = 'মতামত লিখুন।';
        errorEl.classList.remove('hidden');
        return;
    }
    
    if (text.length < 5) {
        errorText.textContent = 'মতামতটি কমপক্ষে ৫ অক্ষরের হতে হবে।';
        errorEl.classList.remove('hidden');
        return;
    }
    ...
    try {
        ...
        const { data, error } = await supabase
            ...
            .insert({
                ...
                parent_answer_id: state.currentCommentAnswerId,
                body: htmlBody,
        ...
        // Reload comments
        await loadComments(state.currentCommentAnswerId);
        ...
        errorText.textContent = 'মতামত জমা দিতে সমস্যা হয়েছে।';
        errorEl.classList.remove('hidden');
    } finally {
        submitBtn.disabled = false;
