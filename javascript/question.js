...
const createQuestionCard = (question) => {
    ...
    // তোমার প্রোফাইল টেবিলে ছবির কলাম না থাকলে ডিফল্ট ছবি বা নামের প্রথম অক্ষর দেখাবে
    const authorAvatar = question.profile?.avatar_url || `https://ui-avatars.com/api/?name=${authorName}&background=random`;
...
const loadLatestQuestion = async () => {
    ...
    try {
        // এখানে author_id এর জায়গায় তোমার question টেবিলের ইউজার আইডি কলামের নাম দাও
        const { data: questionData, error, count } = await supabase
            .from('question')
            .select('*, profile!inner(full_name, username, avatar_url)', { count: 'exact' }) 
